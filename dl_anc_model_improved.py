...
def main():
    ...
    axes[0].plot(time_axis[:show_samples], estimated_signal[:show_samples],
                label=f'{model_name}估计信号', alpha=0.7, color='red')
    axes[0].set_title(f'时域波形对比 (前1000点) - {model_name}')
    ...
    axes[2].set_title(f'PSD对比 (A计权) - {model_name}')
    ...
    plt.tight_layout()
    result_plot_file = f'results/dl_anc_result_{model_name}.png'
    plt.savefig(result_plot_file, dpi=150)
    print(f"结果图已保存: {result_plot_file}")
    plt.show()  # 显示图片
